import random
def generate_test_case_without_cycles(num_gates=1000):
    ...
    for i in range(num_gates):
        ...
        num_input_pins = random.randint(1, min(height, 20))  # Limited to 20 to keep total pins under 40000
        num_output_pins = random.randint(1, min(height, 20))
    ...
    for i in range(len(gates)-1):  # Exclude last gate as source
        ...
        for source_pin_idx in range(len(source_gate['output_pins'])):
            ...
            for _ in range(random.randint(1, 4)):
                if random.random() < 0.4:  # 40% chance to create a connection
    ...
    # Write to file
    with open('random1000.txt', 'w') as f:
        # Write gates
        for gate in gates:
...
if __name__ == "__main__":
    generate_test_case_without_cycles(1000)
